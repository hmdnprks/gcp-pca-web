...
- **Architecture Patterns** — eight end-to-end reference architectures (real-time streaming analytics · batch ETL / data-lake · global scalable web app · event-driven microservices · hybrid enterprise landing zone · cloud migration · ML pipeline · multi-region HA & DR) presented as staged service flows (Ingest → Process → Serve → Archive). Each pattern lists the **use cases that demand it**, the **exam trigger phrases** that point to it, **variants** (swap a service to fit a scenario), common **pitfalls**, and the case studies it appears in. Every service box is clickable and jumps straight to that service's deep-dive node — for building the "systems thinking" the exam rewards over single-service recall.
