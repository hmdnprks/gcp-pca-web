...
- **Architecture Patterns** — eight end-to-end reference architectures (real-time streaming analytics · batch ETL / data-lake · global scalable web app · event-driven microservices · hybrid enterprise landing zone · cloud migration · ML pipeline · multi-region HA & DR) drawn as a **node-link graph**: services are nodes laid out in columns by stage (Ingest → Process → Serve → Archive) and connected by curved, directional SVG edges that show how data flows through the pipeline. Each pattern also lists the **use cases that demand it**, the **exam trigger phrases** that point to it, **variants** (swap a service to fit a scenario), common **pitfalls**, and the case studies it appears in. Every service node is clickable and jumps straight to that service's deep-dive — for building the "systems thinking" the exam rewards over single-service recall.
